...
export class ViewTableTags extends ViewTableTaskCell {
    constructor(row) {
        super(row, "tags");
        this.img_add = $("<img>", {
        ...
        let items = [{ text: "Выберите тег:", is_active: false }];
    ...
    addTrait(data) {
        const trait = new ViewTableTrait({
            ...
            is_removable: true,
            onRemove: async () => {
                if (!this.row.onTagRemove(this.row.data.tags.filter(obj => obj.id != data.id).map(obj => obj.id)))
                    return false;
