import { IViewTableName } from "../interface/name";
import { ViewTableRow } from "./row";
import { ViewTableTask } from "./task";
import { ViewTableTaskCell } from "./task_cell";

export class ViewTableName extends ViewTableTaskCell implements IViewTableName {
    ...
    constructor(row: ViewTableTask) {
        ...
        $(document.body).on("mousemove", (e) => {
            ...
            for(const row of this.row.table.tasks) {
                if(row == this.row || row.parent == this.row || row.depth == 4) continue;
        ...
        this.img_add.on("click", async () => {
            if(!await this.row.onAddSubtask()) return;
        });
        ...
        this.input.on("change", async () => {
            ...
            !await this.row.onNameChange(this.input.val() as string)) return;

            this.row.data.name = this.input.val() as string;            
        });
